...
export default function Product() {
    ...
    <main className="secondary-bg py-40 bg-contain bg-center relative before:absolute before:w-full before:h-full before:top-0 before:left-0 before:bg-[#00000010]" >
                ...
                {suggestedPrds && suggestedPrds.map((prd) => (
                   <div key={prd.id} className="products-card card rounded-2xl relative bg-[#fefcf9] shadow-lg transition-all duration-300 hover:shadow-xl hover:bg-yellow-300 col-span-3 md:col-span-1 border-none">
                        ...
                        <div className="card-actions mt-auto">
                            <Link href={`/products/${prd.id}`} aria-label='Go To Product Page' className="btn shadow-sm px-8 border-none font-semibold rounded-3xl transition-all duration-700 bg-yellow-300 text-[#461e04] mt-3">Discover</Link>
                        </div>
                        </div>
                    </div>
                ))}
